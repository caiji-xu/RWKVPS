...
class ConvXBlock(nn.Module):
    ...
    def __init__(self, dim, ksize, padding, drop_path=0.0, layer_scale_init_value=1e-6):
        super().__init__()
        self.dwconv = nn.Conv2d(
            dim, dim, kernel_size=ksize, padding=padding, groups=dim
        )  # depthwise conv
...
class MultiScaleEncoder(nn.Module):
    def __init__(
        self, in_c, hidden_c=(16, 32, 64), out_c=32, ksize=(3, 5, 7), padding=(1, 2, 3)
    ):
        super().__init__()
