...
class ShiftByConv(nn.Module):
    def __init__(
        ...
        self.dwconv = nn.Conv2d(
            chan, chan, kernel_size=5, stride=1, padding=2, groups=chan
        )
...
class RWKV_Tmix_x060(MyModule):
    def __init__(
        ...
        if shift_type == "conv":
            self.time_shift = ShiftByConv(n_embd)
        else:
            self.shift_pixel = shift_pixel
            self.shift_c_gamma = shift_c_gamma
            self.time_shift = groups_q_shift  # nn.ZeroPad2d((1, -1, 0,

        assert (
            dim_att % self.n_head == 0
        ), f"get `dim_att` = {dim_att} and `n_head`= {self.n_head}"
        ...
        with torch.no_grad():
            ...
            self.time_maa_w2 = nn.Parameter(
                torch.zeros(5, TIME_MIX_EXTRA_DIM, n_embd).uniform_(-0.01, 0.01)
            )

            # fancy time_decay
            decay_speed = torch.ones(dim_att)
            for n in range(dim_att):
                decay_speed[n] = -5 + 8 * (n / (dim_att - 1)) ** (
                    0.7 + 1.3 * ratio_0_to_1
                )
            ...
            self.time_decay_w2 = nn.Parameter(
                torch.zeros(dim_att, TIME_DECAY_EXTRA_DIM).uniform_(-0.01, 0.01)
            )
        ...
        self.ln_x = nn.LayerNorm(
            dim_att, eps=(1e-5) * (head_size_divisor**2)
        )  # nn.GroupNorm(self.n_head, dim_att, eps=(1e-5)*(head_size_divisor**2))

    @MyFunction
    def jit_func(self, x, x_h: int, x_w: int):
        B, C, T = x.size()

        if self.shift_type == "q_shift":
            xx = self.time_shift(
                x, self.shift_pixel, self.shift_c_gamma, x_h, x_w
            )  # - x
        elif self.shift_type == "conv":
        ...
        xxx = (
            torch.tanh(self.time_maa_w1 @ xxx).view(B * T, 5, -1).transpose(0, 1)
        )  # [5*D, C] @ [B, C, T] -> [B, 5*D, T] -> [5, B*T, D]
        xxx = (
            torch.bmm(xxx, self.time_maa_w2).view(5, B, T, -1).transpose(-2, -1)
        )  # [5, B*T, D] @ [5, D, C] -> [5, B, C, T]
...
class RWKV_CMix_x060(MyModule):
    def __init__(
        ...
        self.key = nn.Conv1d(
            n_embd, dim_ffn, 1, groups=4
        )  # nn.Linear(n_embd, dim_ffn, bias=False)
    ...
    def jit_forward(self, x, x_h: int, x_w: int):
        ...
        kv = self.value(k)
        return (
            F.silu(self.receptance(xr)) * kv
        )  # torch.sigmoid(self.receptance(xr)) * kv
...
class RWKV_ChannelMix(MyModule):
    def __init__(
        ...
        self.key = nn.Conv1d(
            n_embd, dim_ffn, groups=groups
        )  # nn.Linear(n_embd, dim_ffn, bias=False)
        self.receptance = nn.Conv1d(n_embd, n_embd, groups=groups)
        self.value = nn.Conv1d(dim_ffn, n_embd, groups=groups)
    ...
    def __init__(
        ...
        *args,
        ...
        if self.layer_id == 0 and pre_ffn > 0:
            ...
            if "x060" in os.environ["RWKV_MY_TESTING"]:
                self.att = RWKV_Tmix_x060(
                    layer_id, head_size_a, dim_att, n_layer, n_embd, head_size_divisor
                )
            elif "x052" in os.environ["RWKV_MY_TESTING"]:
                self.att = RWKV_Tmix_x052(args, layer_id)
            elif "mamba" in os.environ["RWKV_MY_TESTING"]:
                self.att = Mamba(
                    d_model=n_embd, d_state=16, d_conv=4, expand=2.125
                )  # match rwkv6 #params
