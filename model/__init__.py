...
assert len(_all_modules) == len(
    _all_model_class_name
), "length of modules and registry names should be the same"
...
def _active_load_module(module_name, model_class_name):
    ...
    if dict_name in LAZY_LOADER_DICT:
        ...
        logger.error(f"{dict_name} not found in LAZY_LOADER_DICT")
...
def lazy_load_network(
    module_name: str = None, model_class_name: str = None, **kwargs
) -> BaseModel:
    """
