...
from utils import catch_any_error, easy_logger

logger = easy_logger(func_name="RWKVFusion", level="INFO")

#####################################################################################
# history
# 2024-10-24: convert all tensor shape into C-last to align with Vision-RWKV
# 2025-09-11: Reformat all codes
##添加了Spatial mix对modal特征进行了演进，modal分支采用的单独的llm，modal分支llm由二者前置门控，编码层还是原始特征
##353机2025-11-29运行
#
##502机2025-11-29运行
#
...
def window_partition(x, window_size):
    ...
    assert (
        H % window_size == 0 and W % window_size == 0
    ), "image height and width must be divisible by window size"

    # c-last sequence
    windows = rearrange(
        x, "b c (nh p1) (nw p2) -> (b nh nw) c p1 p2", p1=window_size, p2=window_size
    )
    return windows
...
class Modulator(nn.Module):                                         #调制器类初始化
...
class ConditionInput:
    modalities: torch.Tensor | tuple[torch.Tensor, torch.Tensor] = (
        None  # cat two modalities
    )
    llm_feature: torch.Tensor| tuple[torch.Tensor, torch.Tensor] | None = None# llm feature
    mask_input: torch.Tensor = None  # mask input
    '''用户既可以直接传入拼接好的模态数据，也可以传入两个待拼接的模态数据，类内部会自动完成拼接'''
    ...
    def __repr__(self):
        if self.llm_feature is None:
            llm_info = "None"
        elif isinstance(self.llm_feature, tuple):
            # 如果是元组，分别打印两个 LLM 的形状
            llm_info = f"(Main: {self.llm_feature[0].shape}, Modal-Branch: {self.llm_feature[1].shape})"
        else:
            # 如果是单个 Tensor
            llm_info = str(self.llm_feature.shape)
        return (
            f"modalities: {self.modalities.shape}, "
            + f"llm_feature: {llm_info}, "
...
norm_cls_channel_first = partial(
    LayerNorm, LayerNorm_type="WithBias", channel_first=True
)#适用于 通道优先 的特征张量（如形状为 (batch, channels, height, width) 的图像特征）
norm_cls_channel_last = partial(
    LayerNorm, LayerNorm_type="WithBias", channel_first=False
)#适用于 通道最后 的特征张量（如形状为 (batch, height, width, channels) 或 (batch, seq_len, channels) 的序列特征）
...
class MultiModalityFusion(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        evolve_modal: bool = True, # 默认为 True
    ):
        super().__init__()
        ...
        self.evolve_modal = evolve_modal

        assert (
            self.img_ds_r_by_llm >= 1
        ), "img_ds_r_by_llm should be greater than or equal to 1"

        # previous feature encoder对输入图像特征进行卷积提取和维度调整，增强特征表达能力
        ...
        # modalities encoder其他模态编码器（处理非图像模态输入）
        self.modal_convs = nn.Conv2d(# 映射模态特征到n_embd维度
            modal_chan,
            n_embd,
            kernel_size=3,
            padding=1,
        ...
        self.modal_norm = norm_cls_channel_first(n_embd)    # 模态特征归一化
        self.adap_pool = nn.AdaptiveAvgPool2d(1)


        # ============================================================
        # [新增] Modal Update Branch
        # 用于演进辅助特征，保持全分辨率更新
        # ============================================================
        if self.evolve_modal:
            self.modal_update_conv = nn.Sequential(
                nn.Conv2d(n_embd, n_embd, 3, 1, 1, bias=False, padding_mode=CONV_PAD_MODE),
                norm_cls_channel_first(n_embd),
                nn.SiLU() 
            )

            # [新增] LLM 融合门控层
            # 将两个 LLM 特征拼接 (2 * n_embd) -> 投影到 n_embd -> Sigmoid
            self.llm_fusion_gate = nn.Sequential(
                nn.Linear(2 * n_embd, n_embd),
                nn.Sigmoid()
            )
            self.llm_fusion_proj = nn.Linear(2 * n_embd, n_embd, bias=False)
        # ============================================================


        # modals bottleneck
        # botneck_ratio = 2
        self.act_conv = nn.Sequential(          #特征门控机制（动态加权融合图像与模态特征）
        ...
        self.fusion_proj = nn.Sequential(       #融合后特征投影与规范化
            norm_cls_channel_first(n_embd),
            nn.Conv2d(n_embd, n_embd, kernel_size=1),
        )

        # mask prompt encoder掩码特征处理（可选，处理掩码输入）
        ...
        # llm feature encoderLLM 文本特征处理（可选，融合文本信息）
        if llm_chan is not None:
            self.with_llm_feat = True

            # ======================= START: 添加的代码 =======================
            # 定义一个可学习的张量 (random 语言特征)
            # 维度为 (1, 1, n_embd) 以便广播到 (B, L, n_embd)
            self.random_lang_feat = nn.Parameter(torch.zeros(1, 512, n_embd))
            # 使用截断正态分布进行随机初始化
            trunc_normal_(self.random_lang_feat, std=0.02)
            # ======================= END: 添加的代码 =========================
            ...
            if self.add_mm_tokens:
                assert (
                    self.llm_chan is not None
                ), "add_mm_tokens must be True when using llm feature"
            ...
            self.llm_dense = nn.Sequential(
                norm_cls_channel_last(llm_chan), nn.Linear(llm_chan, n_embd, bias=False)
            )
            ...
            # multi-modal RWKV attention用于图像与文本特征的交互
            self.multi_modal_attn = RWKV_TMix(
                dim_att, # 注意力维度（=n_embd）
                N_HEAD,  #注意力头数（n_embd // HEAD_SIZE）
                n_layer, # 总层数
                layer_id,
                ...
                with_cp=checkpoint,
                img_txt_cat_order=self.txt_cat_order, # 图像与文本的拼接顺序（0或1）
                scan_K=K,
                decay_speed=T_MIX_DECAY_SPEED,
            )
            self.multi_modal_img_ln = norm_cls_channel_last(n_embd) # 图像特征进入注意力前的归一化
            self.multi_modal_txt_ln = norm_cls_channel_last(n_embd) # 文本特征进入注意力前的归一化

            # img and txt out norm
            # self.llm_out_ln = norm_cls_channel_first(n_embd)

            # image and text mlps融合后特征的非线性转换
            ffn_cls = FeedForwardLLaMA if ffn_type == "llama" else FeedForwardQWen2
            self.img_mlp = ffn_cls(
                n_embd, n_embd, multi_modal_mlp_hidden_ratio, feature_ndim=4
            )
            if not self.last_enc_block:
                self.txt_mlp = ffn_cls(
                    n_embd, n_embd, multi_modal_mlp_hidden_ratio, feature_ndim=3
                )

            # ======================= START: 新增 Modal-LLM 独立交互分支 =======================
            if self.evolve_modal:
                # 1. Modal 特征的序列化前处理 (Conv2d 投影)
                #    注意：MIFM_modal_chan 已经是 n_embd (在演进模式下)，所以这里可能是 n_embd -> n_embd
                #    如果需要处理不同维度的输入，请根据实际情况调整 in_channels
                self.modal_dense_seq = nn.Sequential(
                    norm_cls_channel_first(n_embd), 
                    nn.Conv2d(n_embd, n_embd, 1, bias=False)
                )

                # 2. 独立的 RWKV_TMix 注意力模块 (用于 Modal + LLM)
                self.modal_llm_attn = RWKV_TMix(
                    dim_att, N_HEAD, n_layer, layer_id,
                    shift_mode=shift_type, attn_bias=attn_bias, key_norm=ATTN_KEY_NORM,
                    with_cp=checkpoint, img_txt_cat_order=self.txt_cat_order,
                    scan_K=K, decay_speed=T_MIX_DECAY_SPEED,
                )

                # 3. 独立的归一化层
                self.modal_branch_modal_ln = norm_cls_channel_last(n_embd)
                self.modal_branch_txt_ln = norm_cls_channel_last(n_embd)

                # 4. 独立的 MLP (用于 Modal 和 该分支的 LLM)
                #    Modal 是图像特征 (ndim=4, 因为会 restore 回 2D)
                self.modal_branch_mlp = ffn_cls(
                    n_embd, n_embd, multi_modal_mlp_hidden_ratio, feature_ndim=4
                )
                if not self.last_enc_block:
                    self.modal_branch_txt_mlp = ffn_cls(
                        n_embd, n_embd, multi_modal_mlp_hidden_ratio, feature_ndim=3
                    )
            # ======================= END: 新增 Modal-LLM 独立交互分支 =========================


            # ESS特征扫描与合并（可能是一种分块处理策略，提升长序列处理效率）
            if scan_mode == "K0":
    ...
    def add_img_pe(self, img, h, w):#给图像和文本特征添加正弦位置编码，增强位置信息感知
        bs = img.size(0)

        if self.img_pe_type == "sine":
            img_pe = torch.from_numpy(get_2d_sincos_pos_embed(self.n_embd, (h, w))).to(
                img
            )
            # add pe
    ...
    def add_llm_pe(self, llm_txt):
        ...
        if self.llm_pe_type == "sine" and llm_txt is not None:
            ...
            llm_pe = torch.from_numpy(
                get_1d_sincos_pos_embed_from_grid(self.n_embd, llm_ids)
            ).to(llm_txt)
            # add pe
    ...
    def extract_img_and_llm_feat(#从注意力输出中分离图像和文本特征（根据是否添加<img>/<txt>标记和拼接顺序调整切片逻辑）。
        self, h: int, w: int, att_or_ffn_out: torch.Tensor, has_llm: bool
    ):
        ...
        if has_llm:
            if self.add_mm_tokens:
                ...
                img_feat, llm_feat = (
                    att_or_ffn_out[:, slice_1],
                    att_or_ffn_out[:, slice_2],
                )
            else:
                if self.txt_cat_order == 0:
    ...
    def downsample_img_by_llm(self, img):#下采样图像以匹配文本特征长度，融合后再上采样回原尺寸
        full_size = img.shape[-2:]
        img_ds_size = (
            img.shape[-2] // self.img_ds_r_by_llm,
            img.shape[-1] // self.img_ds_r_by_llm,
        )
        # return F.interpolate(
        #     img,
        #     img_ds_size,
    ...
    def forward(self, img_feat: torch.Tensor, cond_input: ConditionInput):
        modals = cond_input.modalities #其他模态特征（如辅助图像、传感器数据等）
        mask = cond_input.mask_input

        # [修改 1] 解包 LLM 输入：判断是单流（第一层）还是双流（后续层）
        raw_llm_input = cond_input.llm_feature
        if isinstance(raw_llm_input, tuple):
            # 后续层：接收上一层传来的 (Main LLM, Modal LLM)
            llm_feat, llm_in_modal = raw_llm_input
            
        else:
            # 第一层：接收原始 LLM，复制一份给 Modal 分支
            llm_feat = raw_llm_input
            llm_in_modal = raw_llm_input.clone() if raw_llm_input is not None else None
        #llm_feat = cond_input.llm_feature


        # ============================================================
        if modals.shape[1] == self.modal_chan:
            # Case A: 第一层，输入为原始数据 (e.g., 1 channel)
            modals = self.modal_convs(modals)
            modals = self.modal_norm(modals)
        elif modals.shape[1] == self.n_embd:
            # Case B: 后续层，输入为已演进特征 (e.g., 64 channels)
            # 归一化
            modals = self.modal_norm(modals)
        else:
            # 异常情况保护
            raise ValueError(
                f"Modal channel mismatch: expected {self.modal_chan} (raw) "
                f"or {self.n_embd} (evolved), but got {modals.shape[1]}"
            )
        # ============================================================
        ...
        img_feat_modals = img_feat  # 保存主辅初步融合后的特征（后续可能作为输出）
        ...
        if llm_feat is not None and self.with_llm_feat:
            ...
            llm_feat_o = llm_feat.clone()

            # ======================= START =======================
            # 将可学习的张量 (1, 1, n_embd) 与 llm_feat (B, L, n_embd) 相加
            # PyTorch 的广播机制会自动处理
            llm_feat = llm_feat + self.random_lang_feat
            # ======================= END =========================
            llm_feat = self.add_llm_pe(llm_feat)
            #llm_feat_common = llm_feat.clone()
            
            # Modal Branch LLM 预处理 (仅在演进模式下需要)
            if self.evolve_modal:
                llm_feat_common = self.llm_drop(self.llm_dense(llm_in_modal))
                llm_feat_common_o = llm_feat_common.clone()
                llm_feat_common = self.add_llm_pe(llm_feat_common)
            else:
                llm_feat_common = None

            if self.evolve_modal:# 拼接两个特征
                llm_cat = torch.cat([llm_feat_o, llm_feat_common_o], dim=-1) # (B, L, 2*C)
                # 计算门控系数 (0~1)
                gatem = self.llm_fusion_gate(llm_cat)
                # 推荐：加权平均式
                llm_feat_common = gatem * llm_feat_o + (1 - gatem) * llm_feat_common_o
                llm_feat_common = self.add_llm_pe(llm_feat_common)
                # 或者使用线性投影 (通常效果相似，参数少):
                # llm_feat = self.llm_fusion_proj(llm_cat)
            else:
                llm_feat_common = llm_feat_common
            # 准备 Img 特征 (下采样 & 序列化)
            # downsample image & ESS
            ...
            img_feat_llm = img_feat     ## 保存融合llm后的主特征




            # ======================= Branch 2: Modal + LLM (新分支) =======================
            if self.evolve_modal:
                # 使用 llm_feat_common 的另一个副本
                llm_feat_branch2 = llm_feat_common

                # 准备 Modal 特征 (假设 modals 已经是 n_embd 通道)
                # 下采样 -> 投影 -> 序列化
                modal_feat, modal_full_size = self.downsample_img_by_llm(modals)
                modal_feat = self.modal_dense_seq(modal_feat) # 投影到序列化空间

                bs_m, C_m, H_m, W_m = modal_feat.shape
                modal_feat = self.scan(modal_feat).view(bs_m * self.K, C_m, H_m * W_m).transpose(1, 2)
                modal_feat = self.add_img_pe(modal_feat, H_m, W_m) # 使用 img PE (因为也是 2D 结构)
                # Norm
                modal_feat_ln = self.modal_branch_modal_ln(modal_feat)
                llm_feat_ln2 = self.modal_branch_txt_ln(llm_feat_branch2)

                # Attention
                attn_out_2 = self.modal_llm_attn(
                    modal_feat_ln, llm_feat_ln2.repeat(self.K, 1, 1), (H_m, W_m)
                )
                modal_feat_out, llm_feat_out_2 = self.extract_img_and_llm_feat(
                    H_m, W_m, attn_out_2, self.with_llm_feat
                )

                # 残差 & 恢复 Modal
                modal_feat = modal_feat + modal_feat_out
                llm_feat_branch2 = llm_feat_branch2 + llm_feat_out_2.view(self.K, bs, -1, C).mean(dim=0)

                modal_feat = rearrange(modal_feat, "(b k) (h w) d -> b k d h w", k=self.K, h=H_m, w=W_m)
                modal_feat = self.merge(modal_feat) / self.K
                modal_feat = self.upsample_img_by_llm(modal_feat, modal_full_size)

                # MLP
                if not self.last_enc_block:
                    modal_feat = self.modal_branch_mlp(modal_feat) + modal_feat
                    llm_feat_branch2 = self.modal_branch_txt_mlp(llm_feat_branch2) + llm_feat_branch2
                
                # 更新输出 modals
                modals = modal_feat
                llm_feat_2 = llm_feat_branch2 # 这是产生的 LLM2

                # 拼接两个特征
                #llm_cat = torch.cat([llm_feat, llm_feat_2], dim=-1) # (B, L, 2*C)
                # 计算门控系数 (0~1)
                # gate 越接近 1，表示越倾向于使用 modal 分支的信息（或自定义逻辑）
                #gate = self.llm_fusion_gate(llm_cat)
                # 推荐：加权平均式
                #llm_feat = gate * llm_feat + (1 - gate) * llm_feat_2


            else:
                # 如果不演进，第二路 LLM 就是原始 LLM (或者 None，取决于您的需求)
                llm_feat_branch2 = llm_feat # 或者 llm_feat_common
            
            # ======================= 返回结果 =======================        
            # 返回:
            # 1. img_feat_modals (门控融合特征)
            # 2. img_feat_llm (Img 分支输出)
            # 3. modals (Modal 分支输出的演进 modal)
            # 4. llm_feat_branch1 (Img 分支演进的 LLM)
            # 5. llm_feat_branch2 (Modal 分支演进的 LLM) <--- 新增返回值
            
            #llm_feat = llm_feat + llm_feat_2
            return img_feat_modals, img_feat_llm, modals, llm_feat,llm_feat_2
        ###########################################################################################

        return img_feat_modals, modals, llm_feat,None
...
class DoubleStreamRWKVBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.roll_size = (
            window_size // 2
            if (scan_id % 2 == 0 and window_size > 0 and not no_roll)
            else 0
        )
        ...
        self.MIFM = MultiModalityFusion(
            ...
            scan_mode=scan_mode,
            attn_bias=attn_bias,
            ffn_type=MIFM_ffn_type,
            shift_type=shift_type,
            n_layer=n_layer,
            add_mm_tokens=add_mm_tokens,
            last_enc_block=last_enc_block,
            evolve_modal=evolve_modal
        )
    ...
    def extract_img_and_llm_feat(
        ...
        if has_llm:
            if self.add_mm_tokens:
                img_feat, llm_feat = (
                    att_or_ffn_out[:, 1 : h * w + 1],
                    att_or_ffn_out[:, h * w + 3 : -1],
                )
            else:
                img_feat, llm_feat = (
                    att_or_ffn_out[:, : h * w],
    ...
    def BRWKV_img_forward(
        ...
        sc1, sh1, ga1 = self.modulator1(x) #缩放 sc1、偏移 sh1、门控 ga1
    ...
    def forward(
        ...
        # MIFM forward
        # 接收 5 个返回值
        MIFM_modals_feat, MIFM_mm_feat, modals, llm_feat, llm_feat_2 = self.MIFM(x, cond)
        # llm_feat_2: 后续会使用

        ## BRWKV forward
        x, llm_feat = self.BRWKV_img_forward(
            x, MIFM_modals_feat, MIFM_mm_feat, llm_feat, patch_resolution
        )
        ...
        # [关键修改] 将两个独立的 LLM 流打包返回,下一层的 DSRB.MIFM 将接收这个元组并解包
        # 如果没有 modal 演进 (llm_feat_modal_branch is None), 则只返回主 LLM
        if llm_feat_2 is not None:
            next_llm_input = (llm_feat, llm_feat_2)
        else:
            next_llm_input = llm_feat

        return x, ConditionInput(modals, next_llm_input, cond.mask_input)
...
class SingleStreamRWKVBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.MIFM = MultiModalityFusion(
            n_embd, MIFM_modal_chan, feat_drop=MIFM_feat_drop,evolve_modal=evolve_modal
        )
...
class FusionSequential(nn.Module):
    def __init__(self, *args, dim: int = None, resi_connection: str = "3conv"):
        super().__init__()
        ...
        if resi_connection == "1conv":
            ...
            self.conv = nn.Sequential(
                nn.Conv2d(dim, r_dim, 3, 1, 1, padding_mode=CONV_PAD_MODE),
            ...
            raise NotImplementedError(
                f"resi_connection {resi_connection} not implemented"
            )
    ...
    def enc_forward(
        self, feat, cond, patch_resolution
    ) -> tuple[torch.Tensor, ConditionInput]:
        outp = feat
...
class UNetDownsample(nn.Module):
    def __init__(self, chan, r=2, chan_r=2, down_type="conv"):
        super().__init__()
        self.norm_scale = nn.Parameter(
            torch.full((chan * chan_r,), r).sqrt()
        )  # amplify the norm scale

        if down_type == "conv":
            self.down = nn.Conv2d(
                chan, chan * chan_r, r, r, bias=False, padding_mode=CONV_PAD_MODE
            )
            ...
            self.down = nn.Sequential(
                nn.Conv2d(
                    chan, dim_mid, 3, 1, 1, bias=False, padding_mode=CONV_PAD_MODE
                ),
                nn.PixelUnshuffle(r),
            )
        else:
...
class UNetUpsample(nn.Module):
    def __init__(
        self, chan, r=2, chan_r=2, up_type="upsample_bilinear", antialias=False
    ):
        super().__init__()
        self.up_type = up_type.split("_")[0]
        self.antialias = antialias
        self.norm_scale = nn.Parameter(
            torch.full((chan // chan_r,), r).rsqrt()
        )  # reduce the norm scale
        self.r = r
        self.resample_mode = (
            up_type.split("_")[1] if self.up_type == "upsample" else None
        )
...
class RWKVFusion(BaseModel):
    ...
    def __init__(
        ...
        pt_img_size = (
            list(pt_img_size)
            if isinstance(pt_img_size, (tuple, list))
            else [pt_img_size, pt_img_size]
        )
        ...
        # flags
        self.has_mask = (
            mask_channel is not None or multi_value_mask_max_classes is not None
        )
        ...
        assert (
            ...
            == len(dec_blk_nums)
            == len(chan_upscales)
            == len(enc_window_sizes)
            == len(dec_window_sizes)
            == len(ffn_hidden_rates)
            == len(MIFM_img_downsample_ratios)
        ), "config length not match"
        ...
        logger.info(
            f"{net_name}: multi-modal settings, input mask: {self.has_mask}, input txt: {self.has_txt}\n"
            f"{net_name}: enc_blk_nums: {enc_blk_nums}, dec_blk_nums: {dec_blk_nums}\n"
            f"{net_name}: chan_upscales: {chan_upscales}, enc_window_sizes: {enc_window_sizes}, dec_window_sizes: {dec_window_sizes}\n"
            f"{net_name}: checkpoint: {with_checkpoint} {'the training will be slow for large batch size' if with_checkpoint else ''}\n"
            f"{net_name}: fusion_prior: {fusion_prior}, use patch merge: {patch_merge}\n"
            f"{net_name}: txt input directly drop ratio: {drop_txt_ratio}, txt embd drop ratio: {txt_embd_drop_ratio}"
            f"{__class__.__name__}: {img_channel=}, {modal_channel=}\n"
        )
        ...
        if llm_channel is not None:
            ...
            txt_pe = torch.from_numpy(
                get_1d_sincos_pos_embed_from_grid(self.llm_channel, txt_ids)
            )  # [l, d]
        ...
        if self.multi_value_mask_max_classes is not None:
            self.mask_embd = nn.Conv2d(
                self.multi_value_mask_max_classes, width, 1, bias=False
            )
            mask_channel = width
        ...
        if end_proj_type == "1conv":
            ...
            self.conv_after_body = nn.Sequential(
                nn.Conv2d(
                    width, bn_chan, 3, 1, 1, bias=False, padding_mode=CONV_PAD_MODE
                ),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(
                    bn_chan, bn_chan, 1, 1, 0, bias=False, padding_mode=CONV_PAD_MODE
                ),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                nn.Conv2d(
                    bn_chan, width, 3, 1, 1, bias=False, padding_mode=CONV_PAD_MODE
                ),
            )
        else:
        ...
        ## fusion head作为模型的最后一步，将经过 conv_after_body处理的特征转换为最终输出格式：重建图像
        self.fusion_head = nn.Conv2d(
            width, out_channel, 3, 1, 1, bias=True, padding_mode=CONV_PAD_MODE
        )
        ...
        inter_dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth // 2)]
        inter_dpr = inter_dpr + inter_dpr[::-1]
        if len(inter_dpr) < depth:
            inter_dpr.append(inter_dpr[-1])
        logger.info(
            f"{net_name}: inter_dpr: {np.array(inter_dpr).round(4)} for depth: {depth}"
        )

        chan = width
        initial_modal_chan = width
        n_prev_blks = 0
        n_prev_layers = 0
        # ============================================================
        # [新增] 跟踪辅助特征通道数的变量
        # 初始值为 width (因为 patch_embd 之后的 modal_in 输出是 width)
        # ============================================================
        current_modal_chan = width 
        # ============================================================
        # encoder
        for layer_id, num in enumerate(enc_blk_nums):
            ...
            for i in range(num):
                encoder_seq.append(
                    DoubleStreamRWKVBlock(
                        ...
                        MIFM_modal_chan=current_modal_chan,
                        evolve_modal=True, # <--- Encoder 启用演进
                        MIFM_mask_chan=mask_channel,
                        MIFM_llm_chan=(llm_channel if n_prev_blks == 0 else prev_chan)
                        if self.has_txt
                        else None,
                        ...
                        MIFM_llm_pe_type="none",
                        MIFM_img_pe_type="none",
                        img_downsample_ratio_by_llm=MIFM_img_downsample_ratios[
                            layer_id
                        ],
            ...
            self.encoders.append(
                FusionSequential(
                    *encoder_seq, dim=chan, resi_connection=layer_resi_connection
                )
            )
            self.downs.append(
                UNetDownsample(
                    chan, r=2, chan_r=chan_upscales[layer_id], down_type="conv"
                )
            )
            # [修改] 在每一层 Encoder 结束时更新 current_modal_chan
            # 因为 block 内部的 MIFM 会把 dim 演进为当前层的 chan
            # 下一层接收到的 modal dim 就是当前层的 chan
            current_modal_chan = chan
        ...
        for dec_layer_id, num in enumerate(reversed(dec_blk_nums)):
            ...
            self.ups.append(
                UNetUpsample(
                    chan,
                    r=2,
                    chan_r=chan_upscales[::-1][dec_layer_id],
                ...
                FusionSequential(
                    ...
                    *[
                        SingleStreamRWKVBlock(
                            ...
                            MIFM_modal_chan=initial_modal_chan,#因为modal不演进，所以维度一直是32，所以会一直触发降/升维操作，此时的current_modal_chan不发挥作用
                            evolve_modal=False, # <--- Decoder 禁用演进
                            MIFM_feat_drop=inter_dpr[n_prev_blks + i],
                            checkpoint=with_checkpoint,
                            no_out_norm=no_out_norm,
                            scan_mode=scan_mode,
            ...
            # [修改] Decoder block 演进后，输出维度变为当前的 chan
            current_modal_chan = chan
    ...
    def resize_pos_embd(self, pos_embd: torch.Tensor, inp_size: tuple):
    ...
    def resize_conds(
        self, H, W, x: torch.Tensor = None, resample_type: str = "bilinear"
    ):
        if exists(x):
    ...
    def _forward_features(self, inp, modal, mask=None, llm_feature=None):
        ...
        # [新增] 存储最初始的辅助特征 (Encoder 演进前的版本备用) 
        initial_modals_for_decoder = u_cond.modalities.clone() 

        ## encoder
        encs = []# 存储编码器的中间特征（用于解码器的跳跃连接）
        ...
        # [修改] 重置 cond.modalities 为 Mid/Decoder 使用的原始特征
        # ============================================================
        # 此时 cond 携带的是 Encoder 最后一层演进后的特征。
  
        if isinstance(u_cond.llm_feature, tuple):
            # 只取主分支 LLM 给 Decoder 使用
            # (Modal 分支的 LLM 在这里完成了它的历史使命)
            final_llm_for_decoder = u_cond.llm_feature[0] 
        else:
            final_llm_for_decoder = u_cond.llm_feature 
        
        # 我们用 initial_modals_for_decoder 替换它，用于 Mid/Decoder。
        u_cond = ConditionInput(
            initial_modals_for_decoder, # 强制使用原始特征
            u_cond.llm_feature, 
            u_cond.mask_input
        )
    ...
    def _forward_recon(self, feature_out, prior, patch_embd_x=None):
        if self.reconstruction_head == "sr":
            ...
            assert (
                patch_embd_x is not None
            ), "patch_embd_x should be provided for fusion reconstruction"
    ...
    def check_multi_modal_inputs(
        ...
        if not self.has_mask:
            ...
            mask = torch.zeros(
                1, self.mask_channel, *vis.shape[-2:], device=device, dtype=dtype
            )
    ...
    def patch_merge_step(self, lms, pan, **kwargs):
        if self.has_txt and "llm_feature" in kwargs:
            ...
            if llm_feature is not None:
                assert (
                    llm_feature.size(0) == 1
                ), "when using PatchMergeModule, input batch size should be 1"
...
class RWKVFusionTester:
    def __init__(
        ...
        cfgs = {
            "pan_small": cfg_pan_small,
            "vif_tiny": cfg_vif_tiny,
            "vif_small": cfg_vif_small,
            "mef_tiny": cfg_mef_tiny,
        }
    ...
    def prepare_data(self, real_img=False):
        ...
        def two_color_fusion(modal_1, modal_2):
            ...
            def back_to_rgb(pred_y):
                tau = 0.5
                mid_1 = cbcr_1 * torch.abs(cbcr_1 - tau) + cbcr_2 * torch.abs(
                    cbcr_2 - tau
                )
        ...
        if real_img:
            img = (
                read_image("/Data4/cao/ZiHanCao/datasets/VIF-MSRS/train/vi/00891N.jpg")[
                    None, ...
                ].to(device)
                / 255.0
            )
            cond = (
                read_image("/Data4/cao/ZiHanCao/datasets/VIF-MSRS/train/ir/00891N.jpg")[
                    None, ...
                ].to(device)
                / 255.0
            )
            ...
            cond = F.interpolate(
                cond, self.img_size, mode="nearest", align_corners=None
            )
            # rgb fn
        ...
        if self.with_mask:
            if real_img:
                mask = read_image(
                    "/Data4/cao/ZiHanCao/datasets/VIF-MSRS/train/Segmentation_labels/00891N.png"
                ).to(device)
                ...
                if self.net.multi_value_mask_max_classes is not None:
                    mask = torch.ones(
                        (
                            self.bs,
                            self.net.multi_value_mask_max_classes,
                            self.img_size[0],
                            self.img_size[1],
                        ),
                        dtype=self.dtype,
                    ...
                    mask = torch.randint(
                        0,
                        9,
                        (self.bs, 1, self.img_size[0], self.img_size[1]),
                        dtype=self.dtype,
                    ).to(img)
        else:
            mask = None
        ...
        if self.with_llm_feature:
            if real_img:
                ...
                llm_file_vi = safe_open(
                    "/Data4/cao/ZiHanCao/datasets/VIF-MSRS/train/t5_feature_MSRS_train_vi.safetensors",
                    framework="pt",
                )
                llm_file_ir = safe_open(
                    "/Data4/cao/ZiHanCao/datasets/VIF-MSRS/train/t5_feature_MSRS_train_ir.safetensors",
                    framework="pt",
                )
                llm_feature_vi = llm_file_vi.get_tensor("00891N").to(
                    device, dtype=self.dtype
                )
                llm_feature_ir = llm_file_ir.get_tensor("00891N").to(
                    device, dtype=self.dtype
                )
                llm_feature = torch.cat([llm_feature_vi, llm_feature_ir], dim=-1)
            else:
                llm_feature = torch.randn(
                    self.bs, 512, self.cfg["llm_channel"], dtype=self.dtype
                ).to(device)
        else:
            llm_feature = None
    ...
    def params_and_flops(self, params_only=True):
        ...
        from model.module.rwkv import vrwkv6_flops
    ...
    def measure_throughput(self, batch_size: int = 56):
        ...
        self.net.forward = self.net._forward_implem
        throghtput = measure_throughput(
            self.net,
            [
                (self.channel_img, self.img_size[0], self.img_size[1]),
    ...
    def test_forward_backward(self, real_img=False):
        ...
        out = self.net._forward_implem(img, cond, mask, llm_feature)
        sr = torch.randn(
            self.bs, self.channel_img, self.img_size[0], self.img_size[1]
        ).to(device)
    ...
    def test_val_step(self):
        if self.task == "pan":
            img, cond, mask, llm_feature, _ = self.prepare_data()
            fused = self.net.sharpening_val_step(
                img, cond, llm_feature, patch_merge=True
            )
            ...
            fused = self.net.fusion_val_step(
                img, cond, mask, llm_feature, to_rgb_fn=to_rgb_fn
            )
            self.info(f"fused shape: {fused.shape}")
            self.summary_mem()
    ...
    def test_fused_optimizer(
        ...
        optimizer = optim.AdamW(
            self.net.parameters(), lr=1e-3, fused=False, foreach=True
        )
...
if __name__ == "__main__":
    ...
    tester = RWKVFusionTester(
        ...
        device=device,
        dtype=torch.float32,
        # pt_path="/Data4/cao/ZiHanCao/exps/panformer/log_file/RWKVFusion_v12_RWKVFusion/vis_ir_joint/2024-11-08-14-44-12_panRWKV_sb45j3w5_vis_ir_joint_rwkv5_2_wo_omnishift_lerp_factor=0/weights/ema_model.pth/model.safetensors",
    )
